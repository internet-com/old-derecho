...
private:
    ...
    /** The node ID that has been assigned to the client that is currently
     * joining, if any. */
    node_id_t joining_client_id;
    /** A cached copy of the last known value of this node's suspected[] array.
     * Helps the SST predicate detect when there's been a change to
     * suspected[].*/
    ...
    /** A flag to signal background threads to shut down; set to true when the
     * group is destroyed. */
    std::atomic<bool> thread_shutdown;
    /** The background thread that listens for clients connecting on our server
     * socket. */
    std::thread client_listener_thread;
    std::thread old_view_cleanup_thread;

    // Handles for all the predicates the GMS registered with the current view's
    // SST.
    ...
    /** Sends a joining node the new view that has been constructed to include
     * it.*/
    ...
    /** Assuming this node is the leader, handles a join request from a
     * client.*/
    void receive_join(tcp::socket &client_socket);

    /** Starts a new Derecho group with this node as the only member, and
     * initializes the GMS. */
    std::unique_ptr<View> start_group(const node_id_t my_id);
    /** Joins an existing Derecho group, initializing this object to participate
     * in its GMS. */
    std::unique_ptr<View> join_existing(const ip_addr &leader_ip,
    ...
    static bool suspected_not_equal(const View::DerechoSST &gmsSST,
                                    const std::vector<bool> &old);
    static void copy_suspected(const View::DerechoSST &gmsSST,
                               std::vector<bool> &old);
    static bool changes_contains(const View::DerechoSST &gmsSST,
                                 const node_id_t q);
    static int min_acked(const View::DerechoSST &gmsSST,
                         const std::vector<bool> &failed);

    /** Constructor helper method to encapsulate creating all the predicates. */
    void register_predicates();

    /** Creates the SST and derecho_group for the current view, using the
     * current
     * view's member list.
     * The parameters are all the possible parameters for constructing
     * derecho_group. */
    ...
    /** Sets up the SST and derecho_group for a new view, based on the settings
     * in
     * the current view
     * (and copying over the SST data from the current view). */
    void transition_sst_and_rdmc(View &newView, int whichFailed);

    /** Lock this before accessing curr_view, since it's shared by multiple
     * threads */
    std::mutex view_mutex;
    /** Notified when curr_view changes (i.e. we are finished with a pending
     * view
     * change).*/
