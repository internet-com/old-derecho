...
private:
     ...
     * in its GMS. */
    std::unique_ptr<View<handlersType>> join_existing(const ip_addr& leader_ip,
    ...
    static void copy_suspected(
        const typename View<handlersType>::DerechoSST& gmsSST,
        std::vector<bool>& old);
    static bool changes_contains(
        const typename View<handlersType>::DerechoSST& gmsSST,
        const node_id_t q);
    static int min_acked(const typename View<handlersType>::DerechoSST& gmsSST,
