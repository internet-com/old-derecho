...
ManagedGroup::ManagedGroup(const int gms_port,
    ...
    auto max_msg_size = DerechoGroup<View::MAX_MEMBERS>::compute_max_msg_size(
        _max_payload_size, _block_size);
    ...
    if(my_id != leader_id && curr_view->vid != 0) {
        ...
        // copy
        // the leader's nChanges and nAcked
...
void ManagedGroup::register_predicates() {
            ...
            if(gmsSST[myRank].suspected[q] && !Vc.failed[q]) {
                ...
                if(Vc.nFailed + 1 >= Vc.num_members / 2) {
                    throw derecho_exception(
                        "Majority of a Derecho group simultaneously "
                ...
                if(Vc.nFailed > Vc.num_members / 2 ||
                   (Vc.nFailed == Vc.num_members / 2 &&
                    Vc.num_members % 2 == 0)) {
                    throw derecho_exception(
                        "Potential partitioning event: this node is "
                        "no longer in the majority and must shut "
                        "down!");
                }
                                     ...
                                     Vc.members[q]))  // Leader initiated
                {
                    ...
                    gmssst::set(gmsSST[myRank].changes[gmsSST[myRank].nChanges %
                                                       View::MAX_MEMBERS],
                                Vc.members[q]);  // Reports the failure (note
                                                 // that q NotIn members)
                    gmssst::increment(gmsSST[myRank].nChanges);
                    //                    std::cout << std::string("NEW
                    //                    SUSPICION: adding
                    //                    ") << Vc.members[q] << std::string("
                    //                    to the
                    //                    CHANGES/FAILED list") << std::endl;
                    log_event(
                        std::stringstream()
    ...
    auto start_join_trig = [this](DerechoSST &sst) {
        ...
        joining_client_socket =
            std::move(pending_joins.locked()
                          .access.front());  // list.front() is now invalid
                                             // because sockets are
                                             // move-only, but C++ leaves it
                                             // on the list
        pending_joins.locked().access.pop_front();  // because C++ list doesn't
        // properly implement queues,
        // this returns void
    ...
    auto start_view_change = [this](DerechoSST &gmsSST) {
        ...
        node_id_t currChangeID =
            gmsSST[myRank].changes[Vc.vid % View::MAX_MEMBERS];
        ...
        auto meta_wedged_continuation = [this, failed, whoFailed](
            DerechoSST &gmsSST) {
            ...
            auto globalMin_ready_continuation = [this, failed, whoFailed](
                ...
                if(IAmTheNewLeader(
                       *curr_view))  // I'm the new leader and everyone who
...
void ManagedGroup::setup_sst_and_rdmc(vector<MessageBuffer> &message_buffers,
    ...
    curr_view->gmsSST = make_shared<sst::SST<DerechoRow<View::MAX_MEMBERS>>>(
    ...
    curr_view->rdmc_sending_group =
        make_unique<DerechoGroup<View::MAX_MEMBERS>>(
            curr_view->members, curr_view->members[curr_view->my_rank],
            curr_view->gmsSST, message_buffers, max_payload_size,
            stability_callbacks, block_size, filename, window_size, 1, type);
}
...
void ManagedGroup::transition_sst_and_rdmc(View &newView, int whichFailed) {
    ...
    //    new_member_map);
    newView.gmsSST = make_shared<sst::SST<DerechoRow<View::MAX_MEMBERS>>>(
        newView.members, newView.members[newView.my_rank],
        [this](const uint32_t node_id) { report_failure(node_id); }, false);
    newView.rdmc_sending_group = make_unique<DerechoGroup<View::MAX_MEMBERS>>(
    ...
    //            gmssst::template
    //            init_from_existing<View::MAX_MEMBERS>(new_row,
    //            old_row);
    //            new_row.vid = newView.vid;
    //        }
    //    }
    // Initialize this node's row in the new SST
    gmssst::template init_from_existing<View::MAX_MEMBERS>(
...
unique_ptr<View> ManagedGroup::join_existing(const ip_addr &leader_ip,
                                             const int leader_port) {
    ...
    for(int i = 0; i < numMembers; ++i) {
        bool success =
            leader_socket.read((char *)&newView->members[i],
...
void ManagedGroup::receive_join(tcp::socket &client_socket) {
    ...
    if((gmsSST[curr_view->my_rank].nChanges -
        gmsSST[curr_view->my_rank].nCommitted) == View::MAX_MEMBERS / 2) {
    ...
    log_event(std::stringstream() << "Proposing change to add node "
                                  << joining_client_id);
    size_t next_change =
        gmsSST[curr_view->my_rank].nChanges % View::MAX_MEMBERS;
...
void ManagedGroup::commit_join(const View &new_view,
...
bool ManagedGroup::suspected_not_equal(const View::DerechoSST &gmsSST,
...
void ManagedGroup::copy_suspected(const View::DerechoSST &gmsSST,
                                  vector<bool> &old) {
...
bool ManagedGroup::changes_contains(const View::DerechoSST &gmsSST,
                                    const node_id_t q) {
...
int ManagedGroup::min_acked(const View::DerechoSST &gmsSST,
...
void ManagedGroup::report_failure(const node_id_t who) {
    ...
    if(cnt >= curr_view->num_members / 2) {
        throw derecho_exception(
            "Potential partitioning event: this node is no "
...
char *ManagedGroup::get_sendbuffer_ptr(unsigned long long int payload_size,
